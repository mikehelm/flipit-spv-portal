...
**One thing for whoever sets up the file storage.** If the storage bucket has *versioning* switched on — a feature some providers offer, which keeps old copies of everything — a deletion writes a note saying "deleted" and quietly keeps the file. Every check above would still pass and every file would still be recoverable. If you use a bucket rather than a disk, it must not be versioned.
