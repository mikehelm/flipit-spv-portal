...
**One thing for whoever sets up the file storage, and it is the one thing none of these checks can see.** If the storage bucket has *versioning* switched on — a feature Amazon, Cloudflare and Backblaze all offer, and which some setup templates switch on for you — then deleting a file writes a note saying "deleted" and quietly keeps the file. The storage would then tell the application the file is gone, every check above would pass, and every file would still be sitting there recoverable by anyone with access to the storage console. An investor who asked to be erased would not have been. **If you use a bucket rather than a disk, turn versioning off** — along with any object lock or retention rule. It is written into `DEPLOYMENT.md §1` and into the configuration file's own notes, because it is a box somebody ticks once and nothing afterwards will ever complain about.
