...
describe('John Doe investor preview', () => {
  ...
  it('defaults text to 50% above the previous size and keeps reliable click controls on every page', () => {
    ...
    expect(control).toContain('COMBINED_PANEL_PREFIXES')
    expect(control).toContain("'/admin'")
    expect(control).toContain("'/portal'")
    expect(control).toContain("'/recipients'")
    expect(control).toContain('pathname.startsWith(`${prefix}/`)')
